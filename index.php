...
<html lang="ko">
	...
	<body>
		...
		<main class="computer_display">
			<div class="main_wrapper">
				<span class="today_webtoon">매일매일 웹툰</span>
				<ul class="main_ba">
					<?php
					$num = 0;
					$result = db_select("SELECT * FROM contents ORDER BY content_update_date DESC");
					foreach($result as $r){
				...
				<div class="recommend_main_header">
					<div class="recomend_main_title"><span>요일별 전체 웹툰</span></div>
					<div class="sort_nav_wrapper">
					<?php 
					if (isset($_GET['popularity']) === false)
					{
						?>
						<div class="sort_nav01"><span>조회순</span></div>
						<a href="index.php?popularity">
					...
					?>
						<a href="index.php">
							<div class="sort_nav02">
								<span>조회순</span>
							</div>
						</a>
						<div class="sort_nav01">
							<span>평점순</span>
						</div>
					<?php 
					}
				...
				<div class="recommend_main_wrapper">
					<div class="recommend_main_content_wrapper">
						...
						for($i = 0; $i <7 ; $i++)
						{
							...
							if (isset($_GET['popularity']) === true)
							{
								$result = db_select("SELECT * FROM contents ORDER BY content_rating DESC");
							}
							foreach($result as $r)
							{
								if("$r[week]" == $i && $r['content_end'] == false && $r['user_toon'] == false)
								{
									?>
		...
		<main class="phone_display">
			<div class="main_wrapper">
				<div class="recommend_main_header">
					<div class="recomend_main_title">
						<span>오늘의 웹툰</span>
					</div>
					<div class="sort_nav_wrapper">
						<?php
						$day = date("Y-m-d");
						$yoil = array(6, 0, 1, 2, 3, 4, 5);
						$dayyoil = ($yoil[date('w', strtotime($day))]);
						if(isset($_GET['popularity']) === false)
						{
							?>
							<div class="sort_nav01">
				...
				<div class="popularity_main_content_wrapper">
					<ul class="recommend_main_contents">
						<?php
						$result = db_select("SELECT * FROM contents WHERE week = $dayyoil ORDER BY content_views DESC");
						if(isset($_GET['popularity']))
						{
							$result = db_select("SELECT * FROM contents WHERE week = $dayyoil ORDER BY content_rating DESC");
						}
						foreach($result as $r)
						{
							if($r['content_end'] == false && $r['user_toon'] == false)
