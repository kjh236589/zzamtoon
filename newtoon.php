...
<!--    <div class="adv_main">상 단 광 고</div>	-->
    ...
    <main class="main_wrapper">
            ...
            <div class="recommend_main_header">
                <div class="recomend_main_title"><span>신규 웹툰</span></div>
            </div>
        <div class="recommend_main_wrapper">
            <div class="popularity_main_content_wrapper">
                <ul class="recommend_main_contents">
                    <?php
					$result = db_select("SELECT * FROM contents ORDER BY content_start_date DESC");
					foreach($result as $r){
						$now = new DateTime();
						$toon = new DateTime($r['content_start_date']);
						$a = $toon->diff($now);
						if((int)$a->format('%a') <= 14 && $r['content_end'] == false && $r['user_toon'] == false){
						?>
